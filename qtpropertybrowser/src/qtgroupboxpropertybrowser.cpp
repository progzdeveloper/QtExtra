...
private:
    ...
    void setVisible(WidgetItem *item, bool visible);
...
void QtGroupBoxPropertyBrowserPrivate::setVisible(WidgetItem *item, bool visible)
{
    if (!item)
        return;
    item->visible = visible;
    if (item->groupBox) {
        item->groupBox->setVisible(visible);
    }
    if (item->label) {
        item->label->setVisible(visible);
    }
    if (item->widgetLabel) {
        item->widgetLabel->setVisible(visible);
    }
    if (item->widget) {
...
void QtGroupBoxPropertyBrowser::setItemVisible(QtBrowserItem * item, bool visible)
{
    QtGroupBoxPropertyBrowserPrivate::WidgetItem *itm = d_ptr->m_indexToItem.value(item);
    if (itm)
        d_ptr->setVisible(itm, visible);
}

bool QtGroupBoxPropertyBrowser::isItemVisible(QtBrowserItem *item) const
{
    QtGroupBoxPropertyBrowserPrivate::WidgetItem *itm = d_ptr->m_indexToItem.value(item);
    if (itm)
        return itm->visible;
